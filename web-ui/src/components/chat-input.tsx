...
export function ChatInput() {
	...
	const createImagePreview = useCallback((file: File): Promise<string | undefined> => {
	...
	const addFiles = useCallback(
		async (files: FileList | Array<File>) => {
			...
			if (currentSize + newSize > MAX_TOTAL_SIZE) {
				alert(`Total attachment size cannot exceed ${MAX_TOTAL_SIZE / 1024 / 1024}MB`);
				return;
			}
	...
	const handleSend = async () => {
		if ((!message.trim() && attachments.length === 0) || !sessionId || isStreaming) return;
		...
		const nonImageFiles = currentAttachments.filter((att) => !att.mimeType.startsWith("image/"));
		...
		try {
			...
			for (const file of nonImageFiles) {
				try {
					const result = await client.uploadAttachment(sessionId, file.file.name, file.base64, file.mimeType);
			...
			await client.prompt(sessionId, finalMessage, images.length > 0 ? images : undefined);
		} catch (err) {
				...
				<div className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-2">
						...
						<p className="text-xs text-muted-foreground">
							<kbd className="rounded bg-muted px-1.5 py-0.5">Ctrl+Enter</kbd> to send
						</p>
					</div>
					...
					<div className="flex items-center gap-2">
						...
						<Button
							onClick={handleSend}
							disabled={(!message.trim() && attachments.length === 0) || !sessionId || isStreaming}
							size="icon"
						>
