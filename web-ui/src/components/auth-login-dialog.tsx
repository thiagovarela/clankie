...
export function AuthLoginDialog({ open, onOpenChange }: AuthLoginDialogProps) {
	...
	useEffect(() => {
		if (open && loginFlow?.status === "complete" && loginFlow.success === true) {
...
function LoginFlowContent({ flow }: { flow: LoginFlowState }) {
	...
	useEffect(() => {
		if (flow.status === "waiting_url" && flow.url && autoOpenedUrlRef.current !== flow.url) {
		...
		return (
			<div className="space-y-4 py-4">
				<div className="flex items-center gap-3">
					<Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
					<span className="text-sm">Complete the authentication in your browser...</span>
				</div>
				...
				<Button onClick={() => window.open(flow.url, "_blank")} className="w-full" variant="outline" size="sm">
