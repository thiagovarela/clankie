...
import { scopedModelsStore } from "@/stores/scoped-models";
...
function parseModelNameVersion(name: string): {
	...
	const cleanName = name.replace(/\s+\((EU|US|Global|latest|free|exacto|thinking|Antigravity)\)$/i, "").trim();
 ...
 */
function groupModelsByProvider(models: Array<ModelInfo>): Map<string, Array<ModelInfo>> {
...
export function ModelSelector() {
	const { sessionId, model, availableModels, thinkingLevel, isStreaming } = useStore(sessionStore, (state) => ({
	...
	const { enabledModels: scopedEnabledModels } = useStore(scopedModelsStore, (state) => ({
		enabledModels: state.enabledModels,
	}));

	const [isOpen, setIsOpen] = useState(false);

	// Compute effective models: filter by scope if active
	const effectiveModels = useMemo(() => {
		if (scopedEnabledModels.length === 0) return availableModels; // no scope = show all

		const scopedSet = new Set(scopedEnabledModels);
		const filtered = availableModels.filter((m) => scopedSet.has(`${m.provider}/${m.id}`));

		// Always include the current model even if not scoped
		if (model && !scopedSet.has(`${model.provider}/${model.id}`)) {
			const currentInAvailable = availableModels.find((m) => m.provider === model.provider && m.id === model.id);
			if (currentInAvailable) filtered.push(currentInAvailable);
		}

		return filtered;
	}, [availableModels, scopedEnabledModels, model]);
	...
	const { latestModels, olderModels, shouldShowSubmenu } = useMemo(() => {
		...
		if (effectiveModels.length <= 10) {
			const sorted = [...effectiveModels].sort((a, b) => a.name.localeCompare(b.name));
			return { latestModels: sorted, olderModels: [], shouldShowSubmenu: false };
		}

		// Otherwise, identify latest and split
		const { latest, older } = identifyLatestModels(effectiveModels);
	...
	const handleThinkingLevelChange = async (level: "off" | "minimal" | "low" | "medium" | "high" | "xhigh") => {
		if (!sessionId) return;
		...
		return (
			...
			>
				...
				<div className="flex-1 min-w-0">
					...
					<div className="text-xs text-muted-foreground line-clamp-2">{getModelDescription(m)}</div>
				</div>
			</DropdownMenuItem>
		);
	};
	...
	return (
		<DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
			<DropdownMenuTrigger
				...
				className={cn(
					"flex items-center gap-2 rounded-lg border border-border/50 bg-background/50 backdrop-blur-sm px-3 py-1.5 text-sm transition-colors",
			...
			>
				<div className="flex flex-col items-start">
					<div className="flex items-center gap-1.5">
						<span className="font-medium truncate max-w-[200px] sm:max-w-[280px]">{model.name}</span>
						<ChevronDown className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
					</div>
					<div className="flex items-center gap-2">
						{thinkingLevel !== "medium" && (
							<span className="text-[10px] text-muted-foreground">thinking: {thinkingLevel}</span>
						)}
						{scopedEnabledModels.length > 0 && (
							<span className="text-[10px] text-muted-foreground flex items-center gap-0.5">
								<Filter className="h-2.5 w-2.5" />
								{scopedEnabledModels.length} scoped
							</span>
						)}
					</div>
				</div>
			...
			<DropdownMenuContent align="center" className="w-80">
				...
				<div className="max-h-96 overflow-y-auto">{latestModels.map(renderModelItem)}</div>
				...
				<DropdownMenuSub>
					...
					<DropdownMenuSubContent>
						{thinkingLevelOptions.map((option) => (
							...
							>
								<div className="flex h-5 w-5 items-center justify-center shrink-0">
									{thinkingLevel === option.value && <Check className="h-4 w-4" />}
								</div>
...
function getModelDescription(model: { provider: string; id: string; name: string }): string {
