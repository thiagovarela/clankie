...
import { makeAuthEventComplete, makeAuthEventPrompt, makeAuthEventUrl, makeAuthProvider } from "@/test/fixtures";
import {
	authStore,
	clearLoginFlow,
	resetAuth,
...
describe("auth store", () => {
	...
	describe("login flow state machine", () => {
		...
		it("url event transitions to waiting_url state", () => {
			startLoginFlow("flow-123", "anthropic");
			updateLoginFlow(makeAuthEventUrl("flow-123", "https://auth.example.com", "Please sign in"));
		...
		it("prompt event transitions to waiting_input state", () => {
			startLoginFlow("flow-123", "anthropic");
			updateLoginFlow(makeAuthEventPrompt("flow-123", "Enter your code", "Code"));
		...
		it("complete event (error) transitions to error state", () => {
			startLoginFlow("flow-123", "anthropic");
			updateLoginFlow(makeAuthEventComplete("flow-123", false, "Authentication failed"));
