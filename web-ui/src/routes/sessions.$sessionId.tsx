...
function SessionPage() {
	...
	useEffect(() => {
		...
		if (activeSessionId === urlSessionId) {
			console.log(`[session-page] Already on session ${urlSessionId}, no switch needed`);
			return;
		}

		console.log(`[session-page] URL sessionId changed to ${urlSessionId}, switching...`);
		...
		return (
			<div className="flex h-full items-center justify-center">
				<div className="text-center space-y-2">
					...
					<p className="text-sm text-muted-foreground">Loading session {urlSessionId}...</p>
				</div>
			</div>
		);
	}
