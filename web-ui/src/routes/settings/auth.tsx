...
import { CheckCircle2, KeyRound, Loader2, Shield, Sparkles, XCircle } from "lucide-react";
...
import { setAvailableModels, sessionStore } from "@/stores/session";
			...
			<div className="h-full flex items-center justify-center chat-background">
				<div className="text-center space-y-4 max-w-md p-8">
					...
					<div className="space-y-2">
						...
						<p className="text-muted-foreground">Connect to clankie to manage AI provider authentication</p>
					</div>
				</div>
			</div>
		);
...
function ProviderAuthSection() {
	const { providers, isLoadingProviders, loginFlow } = useStore(authStore, (state) => ({
				...
				if (client) {
					client
						...
						.then(({ models }) => {
							...
							console.log("[settings/auth] Refreshed available models after OAuth login");
						})
						.catch((err) => {
							console.error("[settings/auth] Failed to refresh available models:", err);
						});
				}
			}
		}
			...
			<div className="container max-w-2xl py-8 px-4">
					...
					<CardContent>
						{isLoadingProviders ? (
							...
							<p className="text-sm text-muted-foreground py-4">
								No providers available. Make sure clankie is configured with at least one AI provider.
							</p>
						) : (
				...
				<Card className="mt-4 card-depth">
					...
					<CardContent className="space-y-3 text-sm text-muted-foreground">
						<p>AI providers require authentication to access their APIs. You can authenticate using:</p>
						<ul className="list-disc list-inside space-y-1">
							<li>
								<strong>OAuth</strong> - Browser-based authentication flow for supported providers
							</li>
							<li>
								<strong>API Key</strong> - Direct API key entry for providers that support it
							</li>
						</ul>
						<p className="text-xs">
							Your credentials are stored securely by clankie and are never shared with the web UI.
						</p>
					</CardContent>
				</Card>
			</div>
	...
	const handleToggleModel = useCallback(
		async (modelValue: string) => {
			...
			try {
				...
				console.error("[ScopedModelsSection] Failed to save scoped models:", err);
	...
	return (
		<Card className="mt-4 card-depth">
			<CardHeader>
				...
				<CardDescription>
					Choose which models are available in the model selector. Leave empty to show all available models.
				</CardDescription>
			</CardHeader>
			<CardContent>
				{!sessionId ? (
					<p className="text-sm text-muted-foreground">Connect to a session to configure scoped models.</p>
				) : options.length === 0 ? (
					...
					<div className="space-y-4">
						...
						<div className="relative">
							...
							>
								<option value="">{hasSelection ? "Add another model..." : "Select models..."}</option>
								{options
