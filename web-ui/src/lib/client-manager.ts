...
import { clearToolExecutions, hydrateToolExecutionsFromMessages } from "@/stores/tool-executions";
import { handleExtensionUIRequest, resetExtensionUI } from "@/stores/extension-ui";
import {
	resetSession,
	setAvailableModels,
	setModel,
...
import { resetScopedModels, setScopedEnabledModels, setScopedModelsLoading } from "@/stores/scoped-models";
...
class ClientManager {
		...
		try {
			...
			try {
				...
				try {
					...
					const { enabledModels } = await this.client.getScopedModels(result.sessionId);
					setScopedEnabledModels(enabledModels);
				} catch (err) {
					console.error("[client-manager] Failed to fetch scoped models:", err);
				} finally {
	...
	async switchSession(sessionId: string): Promise<void> {
		...
		try {
			...
			const assistantToolCallSummary = messages
				...
				.map((msg, index) => {
					...
					return {
						...
						toolCallIds: content.filter((block: any) => block?.type === "toolCall").map((block: any) => block?.id),
					};
				});
			console.log("[client-manager] assistant content/toolCall summary:", assistantToolCallSummary);
			...
			try {
				setScopedModelsLoading(true);
				const { enabledModels } = await this.client.getScopedModels(sessionId);
				setScopedEnabledModels(enabledModels);
			} catch (err) {
				console.error("[client-manager] Failed to fetch scoped models:", err);
			} finally {
