...
export class ClankieClient {
	...
	async getScopedModels(sessionId: string): Promise<{ enabledModels: string[] }> {
		const response = await this.sendCommand({ type: "get_scoped_models" }, sessionId);
		return response as { enabledModels: string[] };
	}

	async setScopedModels(sessionId: string, models: string[]): Promise<{ enabledModels: string[] }> {
		const response = await this.sendCommand({ type: "set_scoped_models", models }, sessionId);
		return response as { enabledModels: string[] };
	}
	...
	async getExtensionConfig(
		...
		const response = await this.sendCommand({ type: "get_extension_config", extensionPath }, sessionId);
		return response as {
			extensionPath: string;
	...
	async setExtensionConfig(
		...
		const response = await this.sendCommand({ type: "set_extension_config", extensionPath, config }, sessionId);
		return response as {
			extensionPath: string;
			config: Record<string, unknown>;
		};
	...
	async extensionUIAction(
		...
		const response = await this.sendCommand({ type: "extension_ui_action", extensionPath, action, params }, sessionId);
		return response as {
			extensionPath: string;
			action: string;
			config: Record<string, unknown>;
